...
func init() {
	android.RegisterModuleType("axiom_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = axiomExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = axiomExpandVariables(ctx, dep_file)
	...
	cmd := axiomExpandVariables(ctx, String(g.properties.Cmd))
