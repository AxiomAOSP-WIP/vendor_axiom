...
ifeq "$(wildcard $(KERNEL_SRC) )" ""
    ...
    ifeq ($(TARGET_KERNEL_CONFIG),)
        ...
        ifneq ($(TARGET_FORCE_PREBUILT_KERNEL),)
            ifneq ($(filter RELEASE NIGHTLY SNAPSHOT EXPERIMENTAL,$(AXIOM_BUILDTYPE)),)
...
define build-image-kernel-modules-axiom
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			($(call build-image-kernel-modules-axiom,$$modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_KERNEL_MODULES_LOAD))); \
			$(if $(BOOT_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-axiom,$$vendor_boot_modules,$(KERNEL_VENDOR_RAMDISK_MODULES_OUT),/,$(KERNEL_VENDOR_RAMDISK_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_RAMDISK_KERNEL_MODULES_LOAD))); \
			) \
		fi
...
$(INSTALLED_DTBIMAGE_TARGET): $(DTC) $(DTB_OUT)
	...
	rm -rf $(DTB_OUT)
>>>>>>> 004a3f2e3 (vendor: build: Wipe DTB/O OBJs before build)
