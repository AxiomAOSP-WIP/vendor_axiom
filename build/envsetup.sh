function __print_axiom_functions_help() {
cat <<EOF
...
- axiomgerrit:   A Git wrapper that fetches/pushes patch from/to AxiomOS Gerrit Review.
- axiomrebase:   Rebase a Gerrit change and push it again.
- axiomremote:   Add git remote for AxiomOS Gerrit Review.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Axiom model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch axiom_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/axiom-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.axiom.device | grep -q "$AXIOM_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AXIOM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function axiomremote()
{
    ...
    git remote rm axiom 2> /dev/null
    ...
    local AXIOM_USER=$(git config --get review.review.axiomos.org.username)
    if [ -z "$AXIOM_USER" ]
    then
        git remote add axiom ssh://review.axiomos.org:29418/$PFX$PROJECT
    else
        git remote add axiom ssh://$AXIOM_USER@review.axiomos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'axiom' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.axiom.device | grep -q "$AXIOM_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        axiomremote
        git push axiom HEAD:refs/heads/'$1'
    fi
    '
}

function axiomgerrit() {
    ...
    local user=`git config --get review.review.axiomos.org.username`
...
function axiomrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      axiomrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.axiomos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.axiom.device | grep -q "$AXIOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/axiom/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AXIOM_FIXUP_COMMON_OUT ]; then
